...
describe('resolveCardCallbackOperatorId', () => {
  it('returns open_id when both fields are present (Schema 1 default)', () => {
    expect(resolveCardCallbackOperatorId({ open_id: 'ou_abc', user_id: 'uid_123' })).toBe('ou_abc');
  });

  it('returns open_id when only open_id is present', () => {
    expect(resolveCardCallbackOperatorId({ open_id: 'ou_abc' })).toBe('ou_abc');
  });

  it('falls back to user_id when open_id is absent (Schema 2)', () => {
    expect(resolveCardCallbackOperatorId({ user_id: 'uid_123' })).toBe('uid_123');
  });
  ...
  it('returns undefined when both fields are empty strings', () => {
    ...
    expect(resolveCardCallbackOperatorId({ open_id: '', user_id: '' })).toBe('');
  });

  it('prefers non-empty open_id over non-empty user_id', () => {
    expect(resolveCardCallbackOperatorId({ open_id: 'ou_real', user_id: 'uid_fallback' })).toBe('ou_real');
  });
  ...
  it('skips empty-string open_id and returns user_id (Schema 2 edge case)', () => {
    ...
    expect(resolveCardCallbackOperatorId({ open_id: '', user_id: 'uid_123' })).toBe('uid_123');
  });
});
