...
function buildCompleteCard(params: {
  ...
  if (showUnifiedPanel) {
    ...
    if (hasReasoning) {
      ...
      children.push({
        tag: 'markdown',
        content: `<font color='grey'>💭 ${dur?.zh ?? '思考过程'}</font>`,
        text_size: 'notation',
      });
      children.push({
        tag: 'markdown',
    ...
    if (toolUseSteps && toolUseSteps.length > 0) {
      children.push(...toolUseSteps.flatMap((step) => buildToolUseStepElements(step)));
    }
    if (children.length === 0) {
      children.push(buildToolUsePlaceholder({ zh: '暂无思考与工具调用过程', en: 'No reasoning or tool activity' }));
    }
    ...
    elements.push({
      ...
      header: {
        title: {
          tag: 'plain_text',
          content: parts.join(' · '),
          i18n_content: { zh_cn: parts.join(' · '), en_us: parts.join(' · ') },
        },
