...
import { createReplyPrefixContext, createTypingCallbacks } from '../core/channel-message-compat';
...
export function createFeishuReplyDispatcher(params: CreateFeishuReplyDispatcherParams): FeishuReplyDispatcherResult {
  ...
  log.info('reply mode resolved', {
    ...
    replyMode,
    chatType,
    probeFeishuReplyMode: feishuCfg?.replyMode,
    probeCfgHasChannels: !!(cfg as any)?.channels?.feishu,
    probeCfgReplyMode: (cfg as any)?.channels?.feishu?.replyMode,
    probeCfgKeys: Object.keys((cfg as any)?.channels?.feishu ?? {}).join(','),
  });
  ...
  // 🍤 fry UX：派发即建卡——不等首个增量，立刻展示"处理中"卡片。
  // ensureCardCreated 幂等（状态机守护），后续增量事件不会重复建卡。
  if (controller) {
    void controller.ensureCardCreated().catch((err) => {
      log.warn('eager card creation failed', { error: String(err) });
    });
  }
