...
| ② 模式     | 不需要（2.0.6 起 auto 默认流式） | 同左                     | **群聊与私聊默认一致出卡片**；要群聊安静须显式 `replyMode: { group: "static" }` |
| ③ 工具展示 | 默认开启                         | 默认开启                 | `toolUseDisplay` 不配置即启用（原版默认关闭，本项目已改）                       |

> **群聊 / 私聊行为速查**：
>
> - **私聊 / 群聊**：总开关开着即出入流式卡片（2.0.6 起群聊默认也是卡片——自带卡片引擎的插件理应默认全场景启用）
> - **群聊想安静**：显式 `"replyMode": { "group": "static" }`（群聊静态、私聊仍卡片，按场景区分）
